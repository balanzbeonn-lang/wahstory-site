<?php 
    ...
    if(isset($_POST['UpdateTools'])){
        ...
        $DelResp = $postObj->DeleteAllToolsByUser($Userrow['ClubId']);
        
        $SelectedTools = $_POST['tools'];
        
        $SelectedTools = array_unique($SelectedTools);
        
        if($DelResp == 'success') {
            $_SESSION['responce'] = $postObj->UpdateSelectedTools( $SelectedTools, $Userrow['ClubId']);  
        }
    ...
    if($_SESSION['responce']!=''){
        switch($_SESSION['responce']){
            case 'SUCCESS':
                ...
                header('location: update.tools.php?sucmsg='.$message);
                break;
            case 'ERROR':
      ...
      <div class="col-lg-9 single-profile">
          ...
          <div class="row"> 
            <div class="col-sm-12"> 
              ...
              <div class="d-flex align-items-center justify-content-between">
                <h4 class="mb-2">Update Your Tools</h4>
        ...
        <div class="row">
            <div class="col-md-12">   
            	...
            	<div class="mb-4">
            	        <small class="field-instruction">Add the tools that reflect your abilities</small>
                    
                        <br>
                        
                        <label>Please select upto 6</label>
                    
                    <select id="MultipleToolSelect" class="cs-form_field" name="tools[]" multiple="multiple">
                        
                        <?php
                            $selectedTools = $postObj->GetWAHClubUserToolSById($Userrow['ClubId']);
                            foreach ($selectedTools as $tool1) {
                        ?>
                        
                        <option value="<?=$tool1['tool_id'];?>" selected><?=$tool1['tool'];?></option>
                        
                        <?php 
                            }
                            
                            $tools = $postObj->GetWAHClubAllTools();  
                            if($tools != '') {
                    ...
                    <div class="form_group mt-2">
                        <!--<input type="text" name="otherTools" placeholder="If Other, Enter Tools separated by comma (,) Max: 6" class="otherField cs-form_field" value="" />-->
                    </div>
          ...
          <div class="col-lg-12">  
            ...
            <button class="cs-btn cs-style1" type="submit" name="UpdateTools">
