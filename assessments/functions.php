<?php
require_once(__DIR__ . '/../inc/baseclass.php');

include(__dir__.'/smtpmailfun-test.php');

class Assessment extends BaseClass
{
    ...
    function GetScoreById($id) {
        ...
        $sql = "SELECT * FROM wellness_assessment WHERE id = :id";
        $stmt = $this->openConn->prepare($sql);
        $stmt->execute(['id' => $id]);
        if ($stmt->rowCount()) {
            $data = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    function GetScoreByEmail($email) {
        
        $sql = "SELECT * FROM wellness_assessment WHERE email = :email";
        $stmt = $this->openConn->prepare($sql);
    ...
    function GetScoreBySlug($slug) {
        
        $sql = "SELECT * FROM wellness_assessment WHERE slug = :slug";
        $stmt = $this->openConn->prepare($sql);
    ...
    function updateUserSlug($id, $name){
        
        $slug = $id.'-'.str_replace('', '-', strtolower($name));
        
        $sql = "UPDATE `wellness_assessment` SET `slug` = '$slug' WHERE id = '".$id."'";
    ...
    function GetCountOfTotalUsers(){
        $sql = "SELECT COUNT(*) FROM wellness_assessment";
        $stm = $this->openConn->prepare($sql);
        $stm->execute();
    ...
    function GetAllUsers(){
        $sql = "SELECT * FROM wellness_assessment";
        $stm = $this->openConn->prepare($sql);
        $stm->execute();
        $data = $stm->fetchAll();
        return $data;
    ...
    function GetAverageOfReasonStress($keyword){
        $sql = "SELECT COUNT(*) FROM wellness_assessment WHERE $keyword > 2";
        $stm = $this->openConn->prepare($sql);
        $stm->execute();
        $count = $stm->fetchColumn();
    ...
    function getCountByMultiColumnParam($keyword, $columnName){
        $sql = "SELECT COUNT(*) FROM wellness_assessment WHERE FIND_IN_SET(:keyword, $columnName)";
        $stm = $this->openConn->prepare($sql);
        $stm->execute(['keyword' => $keyword]);
        $count = $stm->fetchColumn();
        return $count ?: null;
    }
    
    function SubmitWellnessAssessmentData($data, $email) {
        
        $sql1 = "SELECT COUNT(*) FROM wellness_assessment WHERE email = :email";
        $stmt = $this->openConn->prepare($sql1);
        $stmt->execute(['email' => $email]);
        if ($stmt->fetchColumn() > 0) {
         ...
         $sql = "INSERT INTO wellness_assessment (addressed_as, full_name, gender, age_group, weight_status, happy_level_yesterday,
        relaxed_level_yesterday, fitness_level_yesterday, excitement_level_yesterday, sports_activity,
        diet_attempt, tried_reduce_alcohol, taken_active_lifestyle, smoking_part_lifestyle,
        alcohol_part_lifestyle, addicted_prescription_drugs, eating_excessively_part_lifestyle,
        eating_little_part_lifestyle, avoiding_foods, diet_balanced, stress_work, stress_finance,
        stress_relationship, stress_health, stress_bereavement, financial_future, ahead_finance,
        money_left, money_relaxed, read_books, time_new_skills, prevent_problems, how_creative,
    ...
    ) VALUES (:addressed_as, :full_name, :gender, :age_group, :weight_status, :happy_level_yesterday,
        :relaxed_level_yesterday, :fitness_level_yesterday, :excitement_level_yesterday, :sports_activity,
        :diet_attempt, :tried_reduce_alcohol, :taken_active_lifestyle, :smoking_part_lifestyle,
        :alcohol_part_lifestyle, :addicted_prescription_drugs, :eating_excessively_part_lifestyle,
        :eating_little_part_lifestyle, :avoiding_foods, :diet_balanced, :stress_work, :stress_finance,
    ...
    $stm = $this->openConn->prepare($sql);
        if($stm->execute($data)) {
            ...
            $NewId = $this->openConn->lastInsertId();
            
            $user = $this->GetScoreById($NewId);
            
            $this->updateUserSlug($NewId, $user['full_name']);
            
            $userRow = $this->GetScoreById($NewId);
            
            $ScoreURL = 'https://www.wahstory.com/assessments/report.php?fiticon='.$userRow['slug'];
            
            $this->sendResult($userRow['full_name'], $userRow['email'], $ScoreURL);
    ...
    function getScoreGrade($scorevalue){
        if($scorevalue < 40 ) {
	        $Grade = '0, 0, 0, '.$scorevalue.',';
	    } else if($scorevalue > 40 && $scorevalue < 60 ){
	        $Grade = '0, 0, '.$scorevalue.', 0,';
	    } else if($scorevalue > 60 && $scorevalue < 80 ){
	        $Grade = '0, '.$scorevalue.', 0, 0,';
	    } else if($scorevalue > 80 ){
	        $Grade = ''.$scorevalue.', 0, 0, 0,';
	    }
	    return $Grade; 
    }
    
    function getSumOfColumn($column){
        $sql = 'SELECT SUM('.$column.') AS total FROM `wellness_assessment`';
        $stm = $this->openConn->prepare($sql);
        if($stm->execute()) {
    ...
    function getSumOfQues19Value() {
        // The values will be mapped to numeric equivalents
        $valueMap = [
            'All the time' => 0,
            'Most of the time' => 1,
            'Sometimes' => 2,
        ...
        // Create SQL query to get all 'often_stressed' values
        $sql = 'SELECT often_stressed FROM `wellness_assessment`';
        $stm = $this->openConn->prepare($sql);
    
        // Execute the query
        if ($stm->execute()) {
            $total = 0;
            // Loop through each record and sum the mapped values
            while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
                $stressLevel = $row['often_stressed'];
    
                // If the stress level is found in the map, add its numeric value to the total
                if (isset($valueMap[$stressLevel])) {
                    $total += $valueMap[$stressLevel];
                }
            }
            return $total;
        }
    ...
    function getQuesValueCountColumn($column) {
        $sql = 'SELECT '.$column.' FROM `wellness_assessment`';
        $stm = $this->openConn->prepare($sql);
    
        if ($stm->execute()) {
            $totalCount = 0;
    
            while ($userScore = $stm->fetch(PDO::FETCH_ASSOC)) {
                $value = $userScore[$column] ?? '';
                $ques21value = explode(',', (string)$value);
                $ques21value_count = count($ques21value);
                $totalCount += $ques21value_count;
            }
    ...
    function sendResult($MemberName, $email, $ResultsLink) {
        ...
        $maildata["subject"] = "Your Wellness Assessment Result | WAHStory";
...
<head>
  ...
  <title>Your Wellness Assessment Results | WAHStory </title>
  <style type="text/css">
