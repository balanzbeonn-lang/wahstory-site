...
include(__dir__.'/smtpmailfun-test.php');
    ...
    foreach($users as $user) {
       ...
       $ScoreURL = 'https://www.wahstory.com/assessments/report.php?fiticon='.$user['slug'];
       if($user['full_name'] != '' && $user['email'] != '' && $user['slug'] != '') {
            // sendResult($user['full_name'], $user['email'], $ScoreURL);
            echo 'Sent to '.$user['full_name'].'<br>';
       } else {
           echo 'Not sent to '.$user['full_name'].'<br>';
       }
