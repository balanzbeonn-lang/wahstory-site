...
use App\Models\Custom_availability;
use App\Models\Availability_timezone;
...
class StepFormController extends Controller
{
    ...
    public function getUserPortfolioBySlug($slugUsername)
    {
        ...
        $customavailabilities = Custom_availability::where('user_id', $user->id)->get();
        
        if ($availabilities->isEmpty()) {
            $availabilities = 'No availabilities set'; 
        }
        if ($customavailabilities->isEmpty()) {
            $customavailabilities = ''; // Empty collection
        }
        
        // $availabilities = $availabilities->merge($customavailabilities);

        $mytimezone = NULL;
        $timezoneId = Availability_timezone::where('user_id', $user->id)->first();
        
        if ($timezoneId) {
            $mytimezone = Timezone::where('id', $timezoneId->timezone_id)->first();
        }

         
        
        $bookedslots = Booking::where('member_id', $user->id)->orWhere('user_id', $user->id)->whereIn('status', ['Pending', 'Confirmed', 'Completed'])->get();
        
        return view('mypresence', compact('user', 'story', 'socials', 'skills', 'tools', 'UserAttributes', 'projects', 'experiences', 'educations', 'awards', 'testimonials', 'blogs', 'loggedinUser', 'connections', 'timezones', 'availabilities', 'customavailabilities', 'mytimezone', 'bookedslots'));
    }
    ...
    public function savedatainsteps(Request $request)
    {
        ...
        while ($request->has("project{$projectCount}name") && !empty($request->input("project{$projectCount}name")) ) {
            ...
            if ($projectId) {
                ...
                $projectsearchCriteria['project_name'] = $projectname;
            }
        ...
        while ($request->has("exp{$experienceCount}company") && !empty($request->input("exp{$experienceCount}company")) ) {
            ...
            if ($experienceId) {
                ...
                $expdata['company_name'] = $experienceCompany;
                $expdata['role'] = $experienceRole;
            } else {
                $searchCriteria['company_name'] = $experienceCompany;
                $searchCriteria['role'] = $experienceRole;
            }
        ...
        $educationCount = 1;
            ...
            if ($EducationId) {
                ...
                $eduData['institution_name'] = $educationUniversity;
                $eduData['degree'] = $educationCourse;
            } else { 
                $EDsearchCriteria['institution_name'] = $educationUniversity;
                $EDsearchCriteria['degree'] = $educationCourse;
            }
        ...
        while ($request->has("awd{$awardCount}title") && !empty($request->input("awd{$awardCount}title")) ) {
            ...
            if ($AwardId) {
                ...
                $awarddata['award_title'] = $awardTitle;
                $awarddata['awarding_body'] = $awardBody;
            } else {
                $AwdsearchCriteria['award_title'] = $awardTitle;
                $AwdsearchCriteria['awarding_body'] = $awardBody;
            }
        ...
        while ($request->has("tstm{$testimonialCount}name") && !empty($request->input("tstm{$testimonialCount}name")) ) {
            ...
            if ($TestimonialId) {
                ...
                $testimonialdata['client_name'] = $testimonialName;
            } else {
                $TstmsearchCriteria['client_name'] = $testimonialName;
            }
        ...
        while ($request->has("blog{$blogCount}title") && !empty($request->input("blog{$blogCount}title")) ) {
            ...
            if ($BlogCountId) {
                ...
                $blogdata['blog_title'] = $blogTitle;
            } else {
                $BlogsearchCriteria['blog_title'] = $blogTitle;
            }
        ...
        if (!empty($request->input('StoryType'))) {
            ...
            if ($request->input('StoryType') == "OLDStory") {
                ...
                $decodedstoryTitle = html_entity_decode($request->input('WAH_storyTitle'), ENT_QUOTES, 'UTF-8');
                
                    $decodedContent = html_entity_decode($request->input('WAH_Story'), ENT_QUOTES, 'UTF-8');
                
                //Story From WAHStory
                $storyData = [
                    'title' => $decodedstoryTitle ?? null,
                    'storycontent' => $decodedContent ?? null,
                ]; 
              ...
              if (!empty($request->input('WAH_storyTitle')) && !empty($request->input('WAH_Story'))) {
                ...
                if ($q1 !== null && $q2 !== null && $q3 !== null && $q4 !== null && $q5 !== null) {
                    ...
                    $P_content = '<p>' . $q1 . '</p> <p> ' . $q2 . '</p> <p> ' . $q3 . '</p> <p> ' . $q4 . '</p> <p> ' . $q5 . '</p>';
                    
                    $decodedstoryTitle = html_entity_decode($request->input('freshstoryTitle'), ENT_QUOTES, 'UTF-8');
                
                    $decodedContent = html_entity_decode($P_content, ENT_QUOTES, 'UTF-8');
                    
                    $storyData['storycontent'] = $decodedContent;
                    $storyData['title'] = $decodedstoryTitle ?? null;
                } else{
                ...
                $decodedstoryTitle = html_entity_decode($request->input('storyTitle'), ENT_QUOTES, 'UTF-8');
                
                $decodedContent = html_entity_decode($request->input('generatedStory'), ENT_QUOTES, 'UTF-8');
     ...
     public function regenerate(Request $request)
    {
        ...
        if (!empty($request->input('ques1')) && !empty($request->input('ques2')) && !empty($request->input('ques3')) && !empty($request->input('ques4')) && !empty($request->input('ques5'))) {
            ...
            $prevFullStory = '<p>'. $previousStory1 .'</p> <p>'. $previousStory2 .'</p> <p>'. $previousStory3 .'</p> <p>'. $previousStory4 .'</p> <p>'. $previousStory5 .'</p>';
            ...
            try {
                ...
                $TitleResponce = OpenAI::chat()->create([
                    ...
                    'messages' => [
                        ...
                        ['role' => 'user', 'content' => "Here is the story: \"$newFullStory\". Please provide a title to this story in 50 to 60 characters"],
                    ],
                ]);
    ...
    */
    ...
    public function bookaslot(Request $request)
    {
      try {
          
        $memberSlug = $request->input('memberslug');
        $slottime = $request->input('slottime');
        $slottimezone = $request->input('slottimezone');
        
        $date = Carbon::parse($slottime);
        $date->setTimezone($slottimezone);
        $slot_Date = $date->format('Y-m-d');
        $slot_start_Time = $date->format('Y-m-d H:i:s');
        $slot_end_Time = $date->addMinutes(30)->format('Y-m-d H:i:s');
        
        
    //Member Details
        $memberRow = User::where('slug_username', $memberSlug)->first();
     ...
     // Check if the logged-in user exists
        ...
        if (!empty($_SESSION['email'])) {
            ...
            $UserBookedAlready = DB::table('bookings')
            ->where(function($query) use ($userId, $slot_start_Time){
                $query->where('user_id', $userId)
                    ->where('start_time', $slot_start_Time)
                    ->whereIn('status', ['Pending', 'Confirmed', 'Completed']);
            })
            ->orWhere(function($query) use ($memberId, $slot_start_Time){
            ...
            ->orWhere(function($query) use ($memberId, $userId, $slot_start_Time){
                $query->where('member_id', $memberId)
                ->where('user_id', $userId)
                ->where('start_time', $slot_start_Time)
                ->whereIn('status', ['Rejected', 'Cancelled', 'Withdrawal']);
            })
            ->count();
            
            if($UserBookedAlready > 0) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Booking declined. Please select a different slot & book again.'
                ]); //if booking Not Available
            }
            
            $UserDayBookingLimit = 1;
            $UserDayBookingCount = DB::table('bookings')
            ->where('user_id', $userId)
            ->where('slot_date', $slot_Date)
            ->whereIn('status', ['Pending', 'Confirmed', 'Completed'])
            ->count();
            if($UserDayBookingCount >= $UserDayBookingLimit) {
                return response()->json([
                    'status' => 'warning',
                    'message' => 'Booking declined. You have already booked a slot for the selected date and can not book another one for the same day.'
                ]); //if booking Limit Exceed
            }
            
            $UserTotalBookingLimit = 5;
            $UserBookingCount = DB::table('bookings')
            ->where('user_id', $userId)
            ->whereIn('status', ['Pending', 'Confirmed', 'Completed'])
            ->count();
            if($UserBookingCount >= $UserTotalBookingLimit) {
                return response()->json([
                    'status' => 'warning',
                    'message' => 'Booking declined. You have reached the maximum booking limit.'.$UserBookingCount
                ]); //if booking Limit Exceed
            }
            
            // Create a new Booking Request with a default status (e.g., 0 - pending)
            $booking = new Booking();
            $booking->user_id = $userId;
            $booking->member_id = $memberId;
            $booking->slot_date = $slot_Date;
            $booking->start_time = $slot_start_Time;
            $booking->end_time = $slot_end_Time;
            $booking->slot_timezone = $slottimezone;
            $booking->status = 'Pending'; // Pending status, you can change it as needed
            $booking->save();
            ...
            $Notification->notification = "You have a new booking request";
            $Notification->isread = 0; // Pending status
            $Notification->link = 'member.bookingrequests.php'; // Pending status
            $Notification->save();
